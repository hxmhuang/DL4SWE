...
def computeInitialCondition(nfile):
    ...
    uc= u0 * np.hstack((-y * np.cos(alpha), \
                        ...
                        -y*np.sin(alpha)))   
    # vectors for translating the field in Cartesian coordinates to a field in spherical coordinates.
...
def computeMetric(Vel, gH, X):
    #global th
    #energy = np.sum(0.5*(Vel**2) - gH)
    energy = np.sum(-gH*(Vel**2 - gH)/2)
    mass   = np.sum(-gH)
    angmom = np.sum(np.cross(X, mass*Vel))
...
tot_inner_energy, tot_mass, tot_angmom = computeMetric(uc, gh, nfile[:,0:3])
print("Total inner energy=",tot_inner_energy, "\tTotal mass=",tot_mass, "\t Total angular momentum=", tot_angmom)
...
with graph.as_default():
    with tf.compat.v1.name_scope("Input"):   
        ...
        weight1 = tf.constant(weight1, dtype=tf.float64, name="weight1")
        weight2 = tf.constant(weight2, dtype=tf.float64, name="weight2")
        ...
        tot_am  = tf.constant(tot_angmom, dtype=tf.float64, name="angularmomentum")
        ...
        t_energy,t_mass,t_angmom=computeMetricTensor(U_next, H_next, X)
...
with tf.compat.v1.Session(graph=graph) as sess:
    ...
    for nt in range(1,100):
        ...
        #t_energy, t_mass, t_angmom = computeMetric(U_train, H_train, nfile[:,0:3])
